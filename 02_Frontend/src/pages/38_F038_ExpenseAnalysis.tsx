...
export default function ExpenseAnalysis() {
  ...
  const buildQS = useCallback(() => {
    ...
    return qs;
  }, [selectedCompanies, year, monthFilter]);
  ...
  return (
    <div className="page-content">
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 22, fontWeight: 700 }}>Expense Analysis</div>
        <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 }}>
          COGS (5xx) + Operating Expenses (6xx)
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16, padding: '12px 16px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20 }}>
          ...
          {/* Month */}
          <div>
            <div style={{ fontSize: 10, color: 'var(--color-text-muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Month</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button style={tabStyle('accounts')} onClick={() => setTab('accounts')}>By Account</button>
        <button style={tabStyle('entity')}   onClick={() => setTab('entity')}>By Entity</button>
        <button style={tabStyle('trend')}    onClick={() => setTab('trend')}>Monthly Trend</button>
      </div>
